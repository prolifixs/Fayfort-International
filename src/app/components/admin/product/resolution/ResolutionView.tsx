...
import type { Request as DatabaseRequest, RequestStatus } from '@/app/components/types/database.types'
...
export function ResolutionView({ product, onStatusChange }: ResolutionViewProps) {
  ...
  const [filteredRequests, setFilteredRequests] = useState<Array<DatabaseRequest & {
    invoice_status: string;
    resolution_status: string;
    notification_sent: boolean;
  }>>(product.requests)
  ...
  const [requests, setRequests] = useState<Array<DatabaseRequest & {
    user: User;
  ...
  const filterRequests = () => {
    let filtered = [...product.requests];  // Create new array to preserve type

    if (statusFilter !== 'all') {
      filtered = filtered.filter(req => req.resolution_status === statusFilter);
    }
  ...
  const handleStatusChange = async (requestId: string, type: 'resolution' | 'invoice', status: string) => {
    try {
      setProcessing(true)
      if (type === 'resolution') {
        await requestProcessor.updateRequestStatus(requestId, status as RequestStatus)
      } else {
        await requestProcessor.updateRequestStatus(
          requestId,
          (status === 'paid' ? 'payment_confirmed' : 'payment_pending') as RequestStatus
        )
      }
